...
export default class mergeLandmassCommand implements IMapTool {
  ...
  async execute(cbk?: (event: any) => any) {
    ...
    const planerGeos = targetGraphics.map((graphic) =>
      graphic.geometry.spatialReference.isGeographic
        ? geographicToWebMercator(graphic.geometry)
        : graphic.geometry,
    )
    const bufferedGeos = planerGeos.map(
      //@ts-ignore
      (geo) => new Polygon({ rings: bufferRings(geo.rings[0], 200) }),
    )
    ...
    const graphic = createMergeGeoGraphic(this._mapContext!, webMercatorToGeographic(finalGeo), { id: uuid('block') })

    // 提交到后台
    const res = await this._cbk({
      action: 'merge',
      sourceGraphics: targetGraphics,
      result: graphic,
    })
    if (res.code !== 0) {
