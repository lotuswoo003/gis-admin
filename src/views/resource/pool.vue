...
const handleRename = async (data: GisResourcePool) => {
  try {
    ...
    if (newName && data.id) {
      // TODO: 调用API更新地块名称
      try {
        const res = await updateResourcePool({ id: data.id, name: newName })
        if (res.code !== 0) {
          ElMessage.error(res.message || '更新地块名称失败')
          return
        }
        ElMessage.success(`地块 "${data.name??'未命名'}" 已重命名为 "${newName}"`)
        loadData() // 重新加载数据
      } catch (error) {
        console.error('更新地块名称失败:', error)
      }
    }
  } catch (error) {
    if (error !== 'cancel') {
